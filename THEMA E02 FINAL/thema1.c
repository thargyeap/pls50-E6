...
void main()
{
   ...
   epivatis bus[53];          /* πίνακας θέσεων */
   ...
   initialize(bus,license, &seats); /* αρχικοποίηση λεωφορείου από το bus.txt */
...
void save_in_file(epivatis bus[], char license[], int seats) {
   ...
   for (i=0;i<seats;i++) { /* για όλες τις θέσεις του λεωφορείου */
      if (bus[i].name[0] != '\0') { /* αν θέση κρατημένη */
         fprintf(f_out,"%s",bus[i].name); /* τύπωσε ον/νυμο */
         fprintf(f_out," %d ", bus[i].seat); /* τύπωσε τη θέση */
         for (j=0;j<10;j++)
            fprintf(f_out,"%1hu",bus[i].tel[j]); /* τύπωσε τηλέφωνο ένα-ένα ψηφίο */
